...
lemma Term.exists_inj_omega :
  t.level ≤ n → ∃ (t' : Term (𝓛^n)), t = t'.injOmega :=
  match t with
  | #x => by intro; exists #x; simp [Term.injOmega]
  | Term.func (n := 0) (Sum.inl f) ([]ₜ) => by
    intro
    exists Term.func (n := 0) (Sum.inl f) ([]ₜ)
  | Term.func (n := 0) (Sum.inr c) ([]ₜ) => by
    intro h
    simp [Term.level] at h
    rcases exists_witness_omega_witness h with ⟨c', h⟩
    exists Term.func (n := 0) (Sum.inr c') ([]ₜ )
    simp [h, Term.injOmega]
  | Term.func (n := n + 1) f ts => by
    intro h; simp [Term.level] at h
    rcases Terms.exists_inj_omega h with ⟨ts', h⟩
    exists Term.func (n := n + 1) f ts'
    simp [h, Term.injOmega]
lemma Terms.exists_inj_omega :
  ts.level ≤ n → ∃ (ts' : Terms (𝓛^n) m), ts = ts'.injOmega :=
  match ts with
  | []ₜ => by intro; exists []ₜ
  | t ∷ₜ ts => by
    intro h
    simp [Terms.level] at h
    rcases h with ⟨h₁, h₂⟩
...
lemma Formula.exists_inj_omega :
  ...
  intro h
  induction p <;> simp [Formula.level] at h
  case atom p ts =>
    rcases Terms.exists_inj_omega h with ⟨ts', h⟩
    exists p ⬝ₚ ts'
...
theorem witness_property_of_W {Γ : Context 𝓛*} :
  ...
  · rcases Formula.exists_inj_omega (p := p) (le_refl _) with ⟨p', h⟩
    let c : Const 𝓛* := Sum.inr ⟨p.level, p'⟩
    exists c
    apply Proof.mp
    · apply Proof.assumption
      apply h₁
