...
def const (𝓢 : Symbol) := 𝓢.𝓕 0

mutual
inductive Term : Symbol → Type where
| var : ℕ → Term 𝓢
| func : 𝓢.𝓕 n → Terms 𝓢 n → Term 𝓢
inductive Terms : Symbol → ℕ → Type where
| nil : Terms 𝓢 0
| cons : Term 𝓢 → Terms 𝓢 n → Terms 𝓢 (n + 1)
end
...
mutual
def Term.vars : Term 𝓢 → Set ℕ
| #x => {x}
| _ ⬝ₜ ts => ts.vars
def Terms.vars : Terms 𝓢 n → Set ℕ
| []ₜ => {}
| t ∷ₜ ts => t.vars ∪ ts.vars
end

def Subst (𝓢) := ℕ → Term 𝓢

mutual
def Term.subst : Term 𝓢 → Subst 𝓢 → Term 𝓢
| #x, σ => σ x
| f ⬝ₜ ts, σ => f ⬝ₜ (ts.subst σ)
def Terms.subst : Terms 𝓢 n → Subst 𝓢 → Terms 𝓢 n
| []ₜ, _ => []ₜ
...
def Subst.comp (σ₁ σ₂ : Subst 𝓢) : Subst 𝓢 := λ x => (σ₁ x)[σ₂]ₜ
...
def Subst.single : Term 𝓢 → Subst 𝓢
| t, 0 => t
| _, x + 1 => #x
prefix:max "↦ₛ " => Subst.single

def Subst.shift : Subst 𝓢 := λ x => #(x + 1)
def Term.shift (t : Term 𝓢) := t[Subst.shift]ₜ
...
def Subst.lift : Subst 𝓢 → Subst 𝓢
| _, 0 => #0
| σ, x + 1 => ↑ₜ(σ x)
...
theorem Subst.lift_id : ⇑ₛ(Subst.id : Subst 𝓢) = Subst.id := by
  funext x
...
inductive Formula : Symbol → Type where
| atom : 𝓢.𝓟 n → Terms 𝓢 n → Formula 𝓢
| false : Formula 𝓢
| implies : Formula 𝓢 → Formula 𝓢 → Formula 𝓢
| all : Formula 𝓢 → Formula 𝓢

namespace Formula
  variable (p q : Formula 𝓢)
  
  infix:70 " ⬝ₚ " => atom
  infixr:55 " ⟶ " => implies
  
  instance : Bot (Formula 𝓢) where
    bot := false
  @[reducible] def neg := p ⟶ ⊥
  prefix:58 "~ " => neg
  instance : Top (Formula 𝓢) where
    top := ~ ⊥
...
def Formula.free : Formula 𝓢 → Set ℕ
| _ ⬝ₚ ts => ts.vars
| ⊥ => {}
| p ⟶ q => p.free ∪ q.free
| ∀' p => {x | x + 1 ∈ p.free}

def Formula.subst : Formula 𝓢 → Subst 𝓢 → Formula 𝓢
...
def Formula.shift : Formula 𝓢 → Formula 𝓢 := λ p => p[Subst.shift]ₚ
